...
if __name__ == "__main__":
    ...
    csv_file = sorted(glob.glob(TOPOLOGICAL_CSV_PATH + '/*.csv'))
    ...
    results = pd.read_csv(csv_file[0])
    features = results.columns.tolist()[2:]
    # print(features)
    ...
    X = results.loc[:,features].to_numpy()
    X = np.asarray(X)
    print(X.shape)
    ...
    # show the plot
    plt.title('Topological clustering of plaques in WSI')
    plt.xlabel('PC1')
    plt.ylabel('PC2')        
    plt.savefig(os.path.join(EXPERIMENT_PATH, 'topological_analysis.png'), dpi=300)
    plt.close()

    cluster_map.to_csv(os.path.join(EXPERIMENT_PATH, 'topological_cluster.csv'))
